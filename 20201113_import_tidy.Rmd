...
crash_map_boro %>% 
  layout(
    title = "Bicycle Crashes with Cyclist Injuries in New York City (2019-2020)")

```

```{r}
crash_map_boro_covid = 
  crash_dat %>% 
  drop_na(c(latitude, longitude, borough)) %>% 
  filter(str_detect(vehicle_options, "[Bb]ike"))  %>% 
  filter(number_of_cyclist_injured > 0)    %>%
...
crash_map_boro_covid %>% 
  layout(
    title = "Cyclist Injuries in New York City Pre and Post Covid")
```
...
crash_map_density %>% 
  layout(
    style = 'open-street-map',
    title = "Density of Bicycle Crashes with Cyclist Injuries in New York City") %>% 
