...
```{r contributing_factor_bike, message = FALSE, warning = FALSE}
bikes = 
...
```{r contributing_factor_micro, message = FALSE, warning = FALSE}
microveh = 
  read_csv("./data/crash_dat.csv") %>%
  filter_for("micro") %>%
  filter(
...
heat_maps = function(dataset, color) {
  pre = 
  dataset %>%
  filter(covid == "Pre-COVID") %>%
  plot_ly(
    x = ~hour, y = ~dow, z = ~collisions, type = "heatmap", colors = color
  ) %>%
  hide_colorbar() %>%
...
subplot(pre, covid, nrows = 2, margin = .1)
}
...
heat_maps(dataset = crash_day_time, color = "BuPu")
```
...
heat_maps(dataset = day_time_bike_err, color = "Blues")
```


Similar to the prior heat maps, crashes with a contributing factor attributed to the cyclist were clustered in the evening hours, with the majority of crashes occurring between 4-6 PM during COVID.
...
heat_maps(dataset = micro_day_time, color = "YlGn")
```
...
heat_maps(dataset = micro_day_time, color = "BuPu")
